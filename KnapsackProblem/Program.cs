...
namespace KnapsackProblem
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] itemNames = new string[] {"", "Tablet", "Camera", "Laptop", "Headphones" };
        ...
        public static int GetMaxValueInContainer(int n, int maxCapacity, int[] weights, int[] values, int[,] data)
        {
            for (int itemNum = 0; itemNum <= n; itemNum++)
            {
                for (int capacity = 0; capacity <= maxCapacity; capacity++)
                {
                    ...
                    {
                        data[itemNum, capacity] = Math.Max(values[itemNum] + data[itemNum - 1, capacity - weights[itemNum]], data[itemNum - 1, capacity]);
                    }
                    else
                    {
        ...
        public static void OutputItemInclusionStatus(int n, int maxCapacity, int[] weights, int[] values, string[] itemNames, int[,] data)
        {
            int i = n;
            int j = maxCapacity;
